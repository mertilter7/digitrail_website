...
export default function ProjectsSection() {
  ...
  return (
    <section id="projects" className="py-20 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-96 h-96 bg-blue-400 rounded-full mix-blend-screen filter blur-xl opacity-10 animate-blob"></div>
      <div className="absolute top-0 right-0 w-96 h-96 bg-purple-400 rounded-full mix-blend-screen filter blur-xl opacity-10 animate-blob animation-delay-2000"></div>
      <div className="absolute -bottom-8 left-20 w-96 h-96 bg-pink-400 rounded-full mix-blend-screen filter blur-xl opacity-10 animate-blob animation-delay-4000"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 mb-16">
          {projects.filter(project => project.id !== 1 && project.id !== 6).map((project, index) => (
            <div 
              ...
              className="group relative bg-slate-950/45 backdrop-blur-sm rounded-3xl p-8 shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border border-white/10 overflow-hidden"
            >
              {/* Background Pattern */}
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className={`px-3 py-1 text-xs font-semibold rounded-full ${
                    project.status === 'Tamamlandı' ? 'bg-green-500/15 text-green-200 border border-green-400/20' :
                    project.status === 'Geliştiriliyor' ? 'bg-blue-500/15 text-blue-200 border border-blue-400/20' :
                    project.status === 'Planlanıyor' ? 'bg-yellow-500/15 text-yellow-200 border border-yellow-400/20' :
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
                      {project.features.map((feature, featureIndex) => (
                        <div key={featureIndex} className="flex items-center space-x-2">
                          ...
                          <span className="text-slate-200 text-xs leading-tight">{feature}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                  ...
                  {project.id === 5 ? (
                    ...
                    <>
                      ...
                      <button 
                        ...
                        className="inline-flex items-center justify-center px-6 py-3 bg-slate-950/60 text-slate-200 border-2 border-white/10 rounded-xl font-semibold hover:bg-white/5 hover:border-white/20 transition-all duration-300 group-hover:scale-105 cursor-pointer"
                      >
                        Detaylı Bilgi Al
                        <svg className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div data-aos="zoom-in-up" className="text-center">
          <div className="bg-slate-950/50 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="border-2 border-blue-400 text-blue-300 px-8 py-4 rounded-xl font-semibold hover:bg-blue-500/15 hover:text-white transition-all duration-200 cursor-pointer"
              >
                WhatsApp&apos;tan Yaz
              </a>
            </div>
